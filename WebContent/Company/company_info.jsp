...
<body>
	...
	<div class="profile">
		<ul class="profile_ul">
			...
			<c:if test="${cnt!=0 }">
				<li class="profile_list"><a href="User?cmd=company_info&cnt=0">내정보</a></li>
			</c:if>
			<c:if test="${cnt==1 }">
				<li class="profile_list" style="border-bottom: 2px solid black;"><strong>정보
						수정</strong></li>
			</c:if>
			<c:if test="${cnt!=1 }">
				<li class="profile_list"><a href="User?cmd=company_info&cnt=1">정보
						수정</a></li>
			</c:if>
			<c:if test="${cnt==2 }">
				<li class="profile_list" style="border-bottom: 2px solid black;"><strong>기업
						정보</strong></li>
			</c:if>
			<c:if test="${cnt!=2 }">
				<li class="profile_list"><a href="User?cmd=company_info&cnt=2">기업
						정보</a></li>
			</c:if>
			<c:if test="${cnt==3 }">
				<li class="profile_list" style="border-bottom: 2px solid black;"><strong>모집
						관리</strong></li>
			</c:if>
			<c:if test="${cnt!=3 }">
				<li class="profile_list"><a href="User?cmd=company_info&cnt=3">모집
						관리</a></li>
			</c:if>
			<c:if test="${cnt==4 }">
				<li class="profile_list" style="border-bottom: 2px solid black;"><strong>지원자
						목록</strong></li>
			</c:if>
			<c:if test="${cnt!=4 }">
				<li class="profile_list"><a href="User?cmd=company_info&cnt=4">지원자
						목록</a></li>
			</c:if>

			<c:if test="${cnt==5 }">
				<li class="profile_list" style="border-bottom: 2px solid black;"><strong>문의
						하기</strong></li>
			</c:if>
			<c:if test="${cnt!=5 }">
				<li class="profile_list"><a href="User?cmd=company_info&cnt=5">문의
						하기</a></li>
			</c:if>
			<c:if test="${cnt==6 || cnt==7 }">
				<li class="profile_list" style="border-bottom: 2px solid black;"><strong>회원
						탈퇴</strong></li>
			</c:if>
			<c:if test="${cnt!=6 and cnt!=7}">
				<li class="profile_list"><a href="User?cmd=company_info&cnt=6">회원
						탈퇴</a></li>
			</c:if>
		</ul>
			...
			<c:if test="${!empty companyinfo }">
				<table class="profile_table">
					<tr class="profile_tbl">
						...
						<td class="profile_name">기업 이름</td>
				...
				<table class="profile_table">
					<tr class="profile_tbl">
						<td class="profile_name01">대표 이름</td>
						<td class="profile_info01">${companyinfo.ceo}</td>
					</tr>
					<tr class="profile_tbl">
						<td class="profile_name01">기업 인증</td>
						<td class="profile_info01"><c:if
								test="${companyinfo.companyck == 0 }">미인증</c:if> <c:if
								test="${companyinfo.companyck == 1 }">인증</c:if></td>
					</tr>
					<tr class="profile_tbl">
					...
					<tr class="profile_tbl">
						<td class="profile_name01">홈페이지</td>
						<td class="profile_info01"><a
							href="http://${companyinfo.homepage}">${companyinfo.homepage}</a></td>
					</tr>
					<tr class="profile_tbl">
						<td class="profile_name01">기업 등급</td>
						<td class="profile_info01">준비중</td>
					</tr>
					<tr class="profile_tbl">
						...
						<tbody>
							...
							<tr>
								<th scope="row"><label for="username"><img
										src="User/Img/red_star_spot.png"> 기업 이름</label></th>
							...
							<tr>
								<th scope="row"><label for="job"><img
										src="User/Img/red_star_spot.png"> 홈페이지</label></th>
							...
							<tr>
								<th scope="row"><label for="hobby"><img
										src="User/Img/red_star_spot.png"> 주소</label></th>
		...
		<c:if test="${cnt == 3 }">
			...
			* 전체 ${totcount } 개의 모집 글을 작성하셨습니다.
			<div class="countck"></div>
				<div class="comm-wrap">
					<table class="tab-list">
						<thead>
							<tr class="tab-01">
								<td class="tab-02">등록 번호</td>
								<td class="tab-02" colspan="1">업종</td>
								<td class="tab-02">고용형태</td>
								<td class="tab-02">지역</td>
								<td class="tab-02">연봉</td>
							</tr>
							<tr>
				...
				<form method="POST" name="question" action="User?cmd=question">
					<table class="tab-list">
						<tbody>
							...
							<tr>
								<th scope="row"><label for="username"><img
										src="User/Img/red_star_spot.png">&nbsp;&nbsp; 제목</label></th>
							...
							<tr>
								<th scope="row"><label for="username"><img
										src="User/Img/red_star_spot.png">&nbsp;&nbsp; 문의 내용</label></th>
							...
							<tr>
								<td colspan="2">
									<div class="join-btn-wrap">
										<input class="join" type="button" value="문의"
		...
		<c:if test="${cnt == 7 }">
			<form action="User?cmd=company_delete" name="userDelete"
				...
				<input class="input-text" type="password" name="pass" id="pass"
					placeholder="8~16자 영문, 숫자, 특수문자"><br>
				<br> <input class="join" type="button" value="탈퇴"
					onclick="sendDelete()">
			</form>
		</c:if>
